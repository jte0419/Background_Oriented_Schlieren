...
%          01/17/21 - Updating pushComputeVideo_Callback based on updated ImageJ macro
%                   - Tested with new ImageJ macro and it works

function varargout = GUI_BOS_v3(varargin)
gui_Singleton = 1;
gui_State = struct('gui_Name',       mfilename, ...
                   'gui_Singleton',  gui_Singleton, ...
                   'gui_OpeningFcn', @GUI_BOS_v3_OpeningFcn, ...
                   'gui_OutputFcn',  @GUI_BOS_v3_OutputFcn, ...
                   'gui_LayoutFcn',  [] , ...
                   'gui_Callback',   []);
...
% --- Executes just before GUI_BOS_v3 is made visible.
function GUI_BOS_v3_OpeningFcn(hObject, eventdata, handles, varargin)
handles.output = hObject;
guidata(hObject, handles);

% --- Outputs from this function are returned to the command line.
function varargout = GUI_BOS_v3_OutputFcn(hObject, eventdata, handles) 
...
% Get raw image file names ready (updated on 01/17/21)
fileRaw = cell(v_numFrames,1);                                              % Initialize raw image cell array
for m = 1:1:v_numFrames                                                     % Loop over all images
    frameNum = m+v_sFrame-1;
    if (frameNum < 10)                                                      % If image number is one digit (based on ImageJ output)
        fileRaw{m} = ['Raw_000' num2str(frameNum) '.tif'];                  % Set appropriate filename
    elseif (frameNum >= 10 && frameNum < 100)                               % If the image number is two digits (based on ImageJ output)
        fileRaw{m} = ['Raw_00' num2str(frameNum) '.tif'];                   % Set appropriate filename
    elseif (frameNum >= 100 && frameNum < 1000)                             % If the image number is three digits (based on ImageJ output)
        fileRaw{m} = ['Raw_0' num2str(frameNum) '.tif'];                    % Set appropriate filename
    elseif (frameNum >= 1000 && frameNum < 10000)                           % If image number is four digits (based on ImageJ output)
